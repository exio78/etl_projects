...
url = "https://runrepeat.com/catalog/womens-running-shoes"
...
for page in range(40): # Iterate over each page, we have 41 pages, index is 0
    ...
    url = "https://runrepeat.com/catalog/womens-running-shoes?page=" + str(next_page)
    print(url)

    # increase the variable for the next
    next_page += 1 
    
# create dataframe 
df = pd.DataFrame(list_shoes_all_pages)

# save data frame as a csv format
df.to_csv("women_running_shoes_two.csv", index = False)